...
<a href="{{ route('competitions.show', $competition->slug) }}"
   {{ $attributes->merge(['class' => 'group bg-card border border-card-border rounded-[var(--radius-base)] shadow-sm hover:shadow-lg hover:border-primary hover:-translate-y-1 transition-all duration-200 overflow-hidden']) }}>
    ...
    <div class="aspect-video bg-gradient-to-br from-primary/10 via-muted to-accent/10 flex items-center justify-center overflow-hidden relative">
        ...
        <div class="absolute top-3 left-3 backdrop-blur-sm bg-background/70 rounded-[var(--radius-base)] px-2.5 py-1 border border-card-border/50">
            <x-badge-category category="tournament" size="sm">
                {{ $competition->federation->name ?? 'PDC' }}
            </x-badge-category>
        </div>
    </div>
    ...
    <div class="p-6 space-y-4">
        ...
        @if(isset($competition->description) && $competition->description)
        ...
        <div class="pt-4 border-t border-border space-y-3">
            {{-- Prize Money --}}
            @if($showPrizeMoney && isset($competition->prize_money) && $competition->prize_money)
            ...
            {{-- Participants Count --}}
            @if($showParticipants && isset($competition->participants_count))
                <div class="flex items-center justify-between">
